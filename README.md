...
- **📊 收盘日报卡片**：顶部三连指标（上证 / 韩国综合指数 / 纳斯达克100），下方持仓列表按当日涨跌幅由大到小排序，展示加粗放大的 T 日场内涨跌及 T-1、T-2 基金净值涨跌幅，红涨绿跌一眼可读。
...
| 模块 | 数据源 | 接口 |
| :--- | :--- | :--- |
| **T日场内行情 / 宽基指数** | 腾讯财经 | `qt.gtimg.cn/q=` |
| **T-1、T-2 基金净值** | 搜狐基金 | `q.fund.sohu.com/q/vl.php` |
...
indices:
  ...
  - { name: "韩国综合指数", symbol_ref: "gzKS11",   flag: "🇰🇷" }
...
`symbol_ref` 沿用腾讯财经规则：A 股 `sh`/`sz` + 6 位代码，美股 `us` + 代码，全球指数使用 `gz` + 指数代码（如韩国综合指数 `gzKS11`）。
